...
size_t Hashtable_Hash(struct Hashtable* ht, const char* key) {
	size_t hash = 5381;
	size_t i, length = strlen(key);
...
struct Mapping* Hashtable_FindMapping(struct Hashtable* ht, const char* key) {
...
void Hashtable_Expand(struct Hashtable* ht) {
	...
	struct Mapping* oldMem = ht->memory;
	size_t i, oldMax = ht->maximum;
	// make new ones
	ht->maximum <<= 1;
	ht->memory = (struct Mapping*)malloc(sizeof(struct Mapping) * ht->maximum);
	Hashtable_Clear(ht);
	ht->count = 0;
...
short Hashtable_Lookup(struct Hashtable* ht, const char* key) {
	struct Mapping* mapping = Hashtable_FindMapping(ht, key);
	if( mapping == NULL )
		return 0;
